...
os.makedirs("saved_models_cropped", exist_ok=True)
...
#generator.load_state_dict(torch.load("/homes/bweinhold/CIS_732/saved_models2/generator_998.pth"))
#discriminator.load_state_dict(torch.load("/homes/bweinhold/CIS_732/saved_models2/discriminator_998.pth"))
#generator.eval()
#discriminator.eval()
...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(gen_imgs.data[:25], "images_cropped/%d.png" % batches_done, nrow=5, normalize=True)

        if batches_done % opt.checkpoint_interval == 0:
            # Save model checkpoints
            torch.save(generator.state_dict(), "saved_models_cropped/generator_%d.pth" % epoch)
            torch.save(discriminator.state_dict(), "saved_models_cropped/discriminator_%d.pth" %epoch)
